  ...
  return (
    ...
    `}>
      ...
      <div className="relative mb-10 z-10">
        <div className={`absolute inset-0 rounded-full blur-2xl animate-pulse-slow ${isDebtMode ? 'bg-red-500/20' : isHrMode ? 'bg-emerald-500/20' : 'bg-cyber-accent/20'}`}></div>
      ...
      <div className="flex items-center justify-between w-full max-w-xs mb-10 relative z-10">
        ...
        <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-full h-0.5 -z-10 
            ${isDebtMode ? 'bg-red-900/50' : isHrMode ? 'bg-emerald-900/50' : 'bg-cyber-700'}
        `}></div>
          ...
          return (
            <div key={s.id} className="flex flex-col items-center gap-2 z-10">
              <div className={`
                ...
                ${isDebtMode ? 'bg-red-950' : isHrMode ? 'bg-emerald-950' : 'bg-cyber-900'}
                ${isActive 
                    ? `${colors.accent} border-current scale-110 shadow-[0_0_10px_currentColor]`
                    : isCompleted 
                        ? isDebtMode ? 'border-red-700 text-red-700' : isHrMode ? 'border-emerald-700 text-emerald-700' : 'border-green-500 text-green-500'
                        : isDebtMode ? 'border-red-900/50 text-red-900' : isHrMode ? 'border-emerald-900/50 text-emerald-900' : 'border-cyber-700 text-slate-600'}
              `}>
              ...
              <span className={`text-[10px] font-mono uppercase tracking-wider absolute -bottom-6 whitespace-nowrap transition-colors duration-300 
                ${isActive 
                    ? colors.accent 
                    : isCompleted 
                        ? isDebtMode ? 'text-red-700' : isHrMode ? 'text-emerald-700' : 'text-green-500' 
                        : 'text-slate-600'}
              `}>
                {s.label}
              </span>
      ...
      <div className="w-full max-w-sm relative z-10 mt-2">
        <div className="flex justify-between text-xs font-mono text-slate-400 mb-2">
             <span className="uppercase">
                {stage === 1 ? (isDebtMode ? t('loading_connect') : isHrMode ? t('loading_connect') : t('loading_connect').split('.')[0] + '...') : 
                 stage === 2 ? (isDebtMode ? t('loading_images_debt') : isHrMode ? t('loading_images_hr') : t('loading_images', { current: 0, total: 0 }).split(':')[0] + '...') : 
                 (isDebtMode ? t('loading_final_debt') : isHrMode ? t('loading_final_hr') : t('loading_final').split('.')[0] + '...')}
             </span>
             <span className={colors.accent}>{Math.round(progress)}%</span>
        </div>
        <div className={`h-1.5 w-full rounded-full overflow-hidden border ${isDebtMode ? 'bg-red-950 border-red-900/50' : isHrMode ? 'bg-emerald-950 border-emerald-900/50' : 'bg-cyber-800 border-cyber-700/50'}`}>
            <div 
